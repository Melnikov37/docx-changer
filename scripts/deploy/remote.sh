...
# Удаленный деплой для GitHub Actions (Docker версия)
# Использование:
...
# Проверка обязательных переменных для CI/CD
if [ -n "$SSH_KEY" ]; then
    # Режим CI/CD - проверяем наличие secrets
...
# Создаем .env локально и копируем на сервер (только в режиме CI/CD)
if [ -n "$SECRET_KEY" ]; then
    echo "📝 Creating .env file..."

    # SSL paths
    # Priority: SSL_CERT content > SSL_CERT_PATH > default Let's Encrypt
    if [ -n "$SSL_CERT" ]; then
        # Certificates passed as content - will be saved to ./ssl/
        ...
        SSL_CERT_FILE="/etc/letsencrypt/live/docxfiller.ru/fullchain.pem"
        SSL_KEY_FILE="/etc/letsencrypt/live/docxfiller.ru/privkey.pem"
    fi
...
"
    ...
    # Передаем SSL сертификаты если есть (содержимое, не пути)
    if [ -n "$SSL_CERT" ] && [ -n "$SSL_KEY" ]; then
        echo "🔒 Uploading SSL certificates..."

        CERT_TMP=$(mktemp)
        KEY_TMP=$(mktemp)
        echo "$SSL_CERT" > "$CERT_TMP"
        echo "$SSL_KEY" > "$KEY_TMP"

        $SCP_CMD "$CERT_TMP" "$USER@$HOST:/tmp/.ssl_cert"
        $SCP_CMD "$KEY_TMP" "$USER@$HOST:/tmp/.ssl_key"
...
echo "📥 Updating on server (Docker deployment)..."
cd $APP_DIR || exit 1
...
# Перемещаем .env если был загружен
...
su - docxapp -c "cd docx-template-filler && mkdir -p data uploads output docx_templates ssl"

# Установка SSL сертификатов если переданы
if [ -f "/tmp/.ssl_cert" ] && [ -f "/tmp/.ssl_key" ]; then
    echo "   Installing SSL certificates..."
    mv /tmp/.ssl_cert "$APP_DIR/ssl/certificate.crt"
    mv /tmp/.ssl_key "$APP_DIR/ssl/private.key"
    chown docxapp:docxapp "$APP_DIR/ssl/certificate.crt" "$APP_DIR/ssl/private.key"
    chmod 600 "$APP_DIR/ssl/certificate.crt" "$APP_DIR/ssl/private.key"
fi
